...
class ChooseVoiceModelView:
    ...
    def __init__(self, root, gender, language, voice_model_service, voice_recordings_service):
        ...
        if self.has_next_page():
            self.next_page_button = tk.Button(self.root, text="Następna strona", width=BUTTON_WIDTH_2, height=BUTTON_HEIGHT_2,
                                        command=self.next_page, font=("Helvetica", 14))
            self.next_page_button.place(x=WIDTH / 2 + 50, y=150 + MODELS_ON_PAGE * PAD_Y)

        main_menu_button = tk.Button(self.root, text="Menu główne", command=self.switch_to_main_menu, width=BUTTON_WIDTH_1, height=BUTTON_HEIGHT_1)
        back_button = tk.Button(self.root, text="Cofnij",width=BUTTON_WIDTH_1, height=BUTTON_HEIGHT_1, command=self.switch_to_choose_gender_language_view)
        continue_button = tk.Button(self.root, text="Dalej",width=BUTTON_WIDTH_1, height=BUTTON_HEIGHT_1, command=self.switch_to_choose_audio_view)
    ...
    def switch_to_choose_gender_language_view(self):
        for widget in self.root.winfo_children():
            widget.destroy()
        choose_gender_language_module.ChooseGenderLanguageView(self.root, self.voice_model_service, self.voice_recordings_service)
    ...
    def display_models(self):
       ...
       for model in models:
           label = tk.Radiobutton(self.root,activebackground='green', highlightthickness=0, highlightcolor='green', text=model["name"], bg='green', font=("Helvetica", 14), variable=self.choosen_model, value=model["id"])
           label.place(x=WIDTH/2-200, y=150+len(self.model_labels*PAD_Y))
           self.model_labels.append(label)
           button = tk.Button(self.root, text="Odsłuchaj", width=BUTTON_WIDTH_2, height=BUTTON_HEIGHT_2,font=("Helvetica", 14), command=lambda: self.play_audio(model['id']))
    ...
    def next_page(self):
        ...
        self.page += 1
        if self.page == 1:
            self.previous_page_button = tk.Button(self.root, text="Poprzednia strona", width=BUTTON_WIDTH_2, height=BUTTON_HEIGHT_2,
