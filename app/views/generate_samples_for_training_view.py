...
class GenerateSamplesView(GenerateSamplesBasicView):
    ...
    def switch_to_previous_view(self):
        for widget in self.root.winfo_children():
            widget.destroy()
        choose_audio_for_training_module.ChooseAudioForTrainingView(self.root, self.voice_model_service, self.voice_recordings_service, self.version_service,self.gender, self.language,
                  self.model_id)
