...
def remove_noise(source, destination):
    ...
    for filepath in paths:
      ...
      tp_s = "audiofiles/datasets/" + destination + "/" + "wavs" + "/"
      tf_s = "audiofiles/datasets/" + destination + "/" + "wavs" + "/" + base
      ...
      subprocess.run(["sox", os.path.join(tempdir.name, "48k.wav"), "-c", "1", "-r", "48000", "-b", "16", 
            "-e", "signed-integer", "-t", "raw", os.path.join(tempdir.name, "temp.raw")]) # convert wav to raw
      ...
      subprocess.run(["sox", "-G", "-v", "0.95", "-r", "48k", "-b", "16", 
            "-e", "signed-integer", os.path.join(tempdir.name,"rnn.raw"), "-t", "wav", os.path.join(tempdir.name,"rnn.wav")]) # convert raw back to wav
      ...
      subprocess.run(["sox", os.path.join(tempdir.name,"rnn.wav"), str(target_file), "remix", "-", "highpass", 
            "100", "lowpass", "7000", "rate", "22050"]) # apply high/low pass filter and change sr to 22050Hz
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog = 'Denoiser',
        description = 'Denoises given sound samples using rnnoise, then places in audiofiles/datasets/<destination>'
        )
    ...
    parser.add_argument('-d', '--destination', action='store', dest='destination', 
        default='dataset', help='Destination dataset name. Default: dataset.')
